...
-  (void)viewWillTransitionToSize:(CGSize)size
  ...
  for (cardView *view in self.cardsViews) {
    ...
    [coordinator animateAlongsideTransition:nil
                                 completion:^(id item){
      CGRect newSize = [self findBound];
      CGPoint newLocation = [self findCenterForCard:view];
      view.bounds = newSize;
      [UIView animateWithDuration:kAnimationTime
                       animations:^{view.center = newLocation;}];
    }];
  }
...
- (CGRect)findBound {
  ...
  CGFloat height = _cardsScreen.bounds.size.height / 8;
